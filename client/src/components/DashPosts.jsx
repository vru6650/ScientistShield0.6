            ...
            <>
              <Table hoverable className='shadow-md'>
                ...
                <Table.Body className='divide-y'>
                  {posts.map((post) => (
                      <Table.Row key={post._id} className='bg-white dark:border-gray-700 dark:bg-neutral'>
                        <Table.Cell>{new Date(post.updatedAt).toLocaleDateString()}</Table.Cell>
                        <Table.Cell>
                          <Link to={`/post/${post.slug}`}>
                            <img src={post.image} alt={post.title} className='w-20 h-10 object-cover bg-neutral' />
                          </Link>
                        </Table.Cell>
                        <Table.Cell>
                          <Link className='font-medium text-text dark:text-white' to={`/post/${post.slug}`}>{post.title}</Link>
                        </Table.Cell>
        ...
        <Modal show={showModal} onClose={() => setShowModal(false)} popup size='md'>
          ...
          <Modal.Body>
            <div className='text-center'>
              ...
              <h3 className='mb-5 text-lg text-muted dark:text-muted'>Are you sure you want to delete this post?</h3>
